...
void Template::LoadChain(const std::string& fname, int depth)
{
	...
	while(std::getline(f, line)) 
	{
		...
		if(!trimmed.empty() && trimmed[0] == '#') line = trimmed;
//		boost::trim(line);
...
std::string Template::GetValue(const std::string& key) const
{
	...
	Frame::const_iterator i = frame_.find(to_lower_copy(key));
...
std::ostream& operator<<(std::ostream& s, const Template& t)
{
	std::string result;
	t.Format(result);
	s << result;
