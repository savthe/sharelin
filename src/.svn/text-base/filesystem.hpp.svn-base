...
void create_file(const std::string&, file_offset_t);
...
void ScanDirectory(const std::string& strPath, OutIter out, Predicate pred, FollowType follow)
{
	...
	for(; i != end; ++i)
	{
		...
		const std::string s = i->path().string();
		if(pred(s))
			*out++ = s;
...
struct FileSizeAtLeast 
{
	FileSizeAtLeast(boost::intmax_t n): sz(n) {}
	bool operator()(const std::string& path) const
	{
		return IsRegularFile(path) && FileSize(path) >= sz;
	}
